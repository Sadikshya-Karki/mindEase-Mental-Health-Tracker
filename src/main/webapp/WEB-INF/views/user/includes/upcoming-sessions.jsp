<%-- Upcoming sessions component: display next scheduled appointments --%>
...
<section class="white-card">
    ...
    <%-- Empty state: shown if no upcoming appointments --%>
    <c:choose>
        <c:when test="${empty upcomingSessions}">
            <p style="color:#9ca3af;">No upcoming sessions yet.</p>
        </c:when>
        <%-- Session rows: counselor name, date, time, and status --%>
        <c:otherwise>
