<%-- Saved resources component: display user's bookmarked resources --%>
...
<section class="white-card">
    ...
    <%-- Empty state: shown if no resources are bookmarked --%>
    <c:choose>
        <c:when test="${empty savedResources}">
            <p style="color:#9ca3af;">No saved resources yet.</p>
        </c:when>
        <%-- Resource rows: display title and category for each saved resource --%>
        <c:otherwise>
