<%-- Registration page: new user account creation entry point --%>
...
<body>
<div class="auth-page">
    <%-- Left panel: branding and marketing benefits shown on desktop --%>
    <aside class="left-panel">
        <div class="blob blob-lavender"></div>
        <div class="blob blob-pink"></div>
        <%-- Brand logo and home nav: positioned in left panel header --%>
      ...
      <a href="${pageContext.request.contextPath}/" style="position:absolute;top:32px;right:28px;display:inline-flex;align-items:center;gap:5px;color:rgba(255,255,255,0.70);font-size:0.82rem;text-decoration:none;background:rgba(255,255,255,0.12);border:1px solid rgba(255,255,255,0.20);border-radius:999px;padding:5px 12px;transition:all 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.20)';this.style.color='#ffffff'" onmouseout="this.style.background='rgba(255,255,255,0.12)';this.style.color='rgba(255,255,255,0.70)'">
        ...
        <div class="left-content">
            ...
            <%-- Feature/benefit list: key selling points for registration --%>
    ...
    <main class="right-panel">
        ...
        <%-- Registration form shell: right-side signup form container --%>
        <div class="form-shell">
            ...
            <%-- Registration form: posts to /register with email, password, and user details --%>
            <form action="${pageContext.request.contextPath}/register" method="post" id="registerForm">
                <%-- Name and phone fields: grid layout for compact form on desktop --%>
                ...
                <div class="field">
                    ...
                    <div style="position:relative;">
                        ...
                        <%-- Password visibility toggle: show/hide button managed by JavaScript --%>
                    ...
                    <%-- Password strength bars: real-time feedback on password complexity --%>
                ...
                <div class="field">
                    ...
                    <div style="position:relative;">
                        ...
                        <%-- Confirm password toggle: synchronized with password field visibility --%>
                        <button type="button" class="eye-toggle" onclick="togglePassword(this)" style="position:absolute;right:12px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:#9ca3af;padding:0;display:none;align-items:center;justify-content:center;width:20px;height:20px;">
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/>
                                <circle cx="12" cy="12" r="3"/>
                ...
                <%-- Password mismatch error: only shown when passwords don't match --%>
...
<script>
    (function () {
        ...
        var currentIndex = 0;
        var intervalId;
        var hasTestimonials = testimonialAvatar && testimonialQuote && testimonialName && dots.length > 0;
        ...
        function evaluatePassword(value) {
            ...
            var meetsLength = value.length >= 6;
            var score = 0;
            ...
            return {
                ...
                label: label,
                color: color,
                valid: meetsLength && hasUpper && hasLower && hasNumber
            };
        }
        ...
        function updateStrength() {
            ...
            var visibleBars = value.length === 0 ? 0 : Math.max(1, evaluation.score);

            for (var i = 0; i < strengthBars.length; i++) {
                strengthBars[i].style.background = i < visibleBars ? evaluation.color : "#e5e7eb";
            }
            strengthLabel.textContent = evaluation.label;
            strengthLabel.style.color = evaluation.color;
        }
        ...
        if (registerForm) {
            registerForm.addEventListener("submit", function (event) {
                var evaluation = evaluatePassword(password.value);
                if (!evaluation.valid) {
                    event.preventDefault();
                    strengthLabel.textContent = "Use 6+ chars with upper, lower, and number.";
